...
export async function generateMetadata(): Promise<Metadata> {
  return { title: "Révision globale — validation" };
}

export default async function AdminGlobalSubmissionReviewPage({ params }: PageProps) {
    ...
    <main className="flex w-full min-h-[calc(100vh-(var(--header-height)+var(--footer-height)))] flex-col">
      <Section fluid classNames="flex min-h-0 flex-1 flex-col bg-pattern py-6">
        <GlobalSubmissionReviewBootstrap
          ...
          lng={lng}
          backHref={`/${lng}/administration/final-validation`}
          isSuperAdmin={true}
        />
      </Section>
    </main>

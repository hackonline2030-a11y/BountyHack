...
type RouteContext = { params: Promise<{ draftId: string; stepKey: string }> };

export async function POST(request: NextRequest, context: RouteContext) {
  const auth = await requireReportWorkflowParticipantBearer();
  if ("response" in auth) return auth.response;

  const { draftId, stepKey } = await context.params;
  const formData = await request.formData();
  const nestRes = await fetch(
    nestInternalApiUrl(
      `report-drafts/draft/${encodeURIComponent(draftId)}/steps/${encodeURIComponent(stepKey)}/attachments/images`,
    ),
    {
      method: "POST",
      headers: {

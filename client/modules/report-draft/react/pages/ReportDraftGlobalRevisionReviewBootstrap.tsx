...
import { ReportDraftGlobalRevisionReviewPage } from "@modules/report-draft/react/pages/ReportDraftGlobalRevisionReviewPage";
...
export const ReportDraftGlobalRevisionReviewBootstrap: React.FC<Props> = ({
  draftId,
  lng,
  backHref,
}) => {
  ...
  if (load.status === "error" || draft === undefined) {
    const message =
      load.status === "error"
        ? load.message
        : `Draft '${draftId}' not loaded.`;
    return <ErrorState message={message} backHref={backHref} />;
  }

  return (
    <ReportDraftGlobalRevisionReviewPage draft={draft} lng={lng} backHref={backHref} />
  );
};
...
const ErrorState: React.FC<{ message: string; backHref: string }> = ({
  ...
  >
    ...
    <Link
      href={backHref}
      className="self-start rounded-md bg-rose-900 px-4 py-2 text-sm font-semibold text-rose-50 transition hover:bg-rose-800"
    >
      ← Retour
    </Link>
  </div>

...
/** `cv.json` is treated as the French/default bundle (`fr`). `cv.<xx>.json` adds other locales. */
const PRIMARY_CV_JSON_LOCALE = 'fr';

const LOCALE_CODE_PATTERN = /^[a-z]{2}$/;
const CVSIDE_FILE_REGEX = /^cv\.([a-z]{2})\.json$/i;
...
@Injectable()
export class JsonCvRepositoryAdapter implements ICvRepository {
  ...
  async listCvVersions(style: string): Promise<string[]> {
    ...
    return Array.from(new Set<string>(versions)).sort(
      (a, b) =>
  ...
  async listCvLocales(style: string, version: string): Promise<string[]> {
    ...
    return this.listCvLocalesForDirectory(versionDir);
  }
  ...
  async getCvTemplateData(
    ...
    for (const candidateStyle of styleCandidates) {
      ...
      const candidateLocales = await this.listCvLocalesForDirectory(candidateDir);
    ...
    const cvAbsolutePath = await this.resolveCvJsonAbsolutePath(
  ...
  private async listCvLocalesForDirectory(
    ...
    const basePath = path.join(versionDirectoryPath, 'cv.json');
    ...
    for (const name of names) {
      const match = CVSIDE_FILE_REGEX.exec(name);
      if (match?.[1]) {
  ...
  private async resolveCvJsonAbsolutePath(
    versionDirectoryPath: string,
    resolvedVersion: string,
    locale: string,
  ): Promise<string> {
    const base = path.join(versionDirectoryPath, 'cv.json');

    if (locale === PRIMARY_CV_JSON_LOCALE) {
      const explicitFr = path.join(versionDirectoryPath, 'cv.fr.json');
      ...
      throw new CvLocaleNotFoundError(locale, resolvedVersion);
    }

    const side = path.join(versionDirectoryPath, `cv.${locale}.json`);

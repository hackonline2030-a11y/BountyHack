/**
 * Depuis la machine hôte : `prisma migrate deploy` vers MySQL Docker (port mappé sur localhost).
 * Réécrit DATABASE_URL (@mysql → 127.0.0.1, port MYSQL_HOST_PORT).
 *
 * Usage (répertoire server/) : `pnpm docker:prisma:migrate:deploy:mysql`
 */
